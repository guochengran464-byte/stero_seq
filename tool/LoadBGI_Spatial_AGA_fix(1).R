...
# fix: bin.coor in cell_mask mode now aggregates by label instead of (x,y)
#      to ensure row count matches hash.B
...
LoadBGI_Spatial <- function(filename,
    ...
    if (cell_mask) {
        ...
        rowname_style <- "CELL"
        colnames(dat)[1:7] <- c("gene", "geneID", "x", "y", "UMICount", "ExonCount", "label")
        if (delete_bg) {
        ...
        # FIX: aggregate by label (not by x,y) so row count matches hash.B
        bin.coor <- dat[, .(x = as.integer(round(mean(x))),
                            y = as.integer(round(mean(y))),
                            V1 = sum(V1)), by = .(label)]
        bin.coor <- bin.coor[order(as.numeric(bin.coor$label)), ]
    }
